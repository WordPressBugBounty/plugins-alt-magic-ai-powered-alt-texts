...
function altm_update_post_without_after_hooks($post_update_data, $context = '') {
    if (!is_array($post_update_data) || empty($post_update_data['ID'])) {
        altm_log('Post update skipped because the update payload is invalid.');
        return false;
    }

    $post_id = absint($post_update_data['ID']);
    $supports_after_hook_suppression = function_exists('wp_after_insert_post') || version_compare(get_bloginfo('version'), '5.6', '>=');

    if ($supports_after_hook_suppression) {
        $result = wp_update_post($post_update_data, true, false);
    } else {
        $result = wp_update_post($post_update_data, true);
    }

    if (is_wp_error($result)) {
        altm_log('Post update failed for post ID ' . $post_id . ($context ? ' (' . $context . ')' : '') . ': ' . $result->get_error_message());
        return false;
    }

    if (!$result) {
        altm_log('Post update returned an empty result for post ID ' . $post_id . ($context ? ' (' . $context . ')' : ''));
        return false;
    }
...
function altm_update_etch_component_alt($component_id, $alt_text, $image_urls) {
    ...
    if ($updated) {
        altm_update_post_without_after_hooks(array(
            'ID' => $component_id,
            'post_content' => $content
        ), 'etch component alt sync');
...
function altm_update_alt_text_in_all_posts($attachment_id, $alt_text) {
    ...
    foreach ($posts as $post) {
        ...
        if ($updated_content !== $post_content) {
            ...
            // This preserves JSON escape sequences like \n, \u002d in Gutenberg blocks
            if (altm_update_post_without_after_hooks(array(
                'ID' => $post->ID,
                'post_content' => str_replace('\\', '\\\\', $updated_content),
            ), 'post-content alt sync')) {
                altm_log('Post updated successfully for post ID: ' . $post->ID);
            } else {
                altm_log('Post update did not complete for post ID: ' . $post->ID);
            }
        } else {
